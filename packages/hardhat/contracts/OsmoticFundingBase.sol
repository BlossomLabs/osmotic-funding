...
contract OsmoticFundingBase is Ownable, FluidFunding {
  ...
  function claim(uint256 _proposalId) external activeProposal(_proposalId) {
    ...
    proposal.balance = 0;
    ERC20(requestToken).safeTransfer(proposal.beneficiary, balance);
  }
  ...
  function calculateTargetRate(uint256 _stake) public view returns (uint256 _targetRate) {
    if (_stake == 0) {
      ...
      uint256 funds = ERC20(requestToken).balanceOf(address(this));
